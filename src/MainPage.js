...
function MainPage({setDisplay, username, sword, setSword}){
    return(
        <div>
            ...
            <button onClick={() => setDisplay(<Sword setDisplay={setDisplay} username={username} sword={sword} setSword={setSword}/>)} disabled={sword}>Obtain A Sword</button>    

            <button onClick={() => setDisplay(<Challenge setDisplay={setDisplay}  username={username} sword={sword}/>)} disabled={!sword}>Challenge the Dragon!</button>                 
        </div>
    )
}
